...
public class OTPGenerator {
	...
	public String generatePassword(String key) throws NoSuchAlgorithmException, InvalidKeyException {
		...
		/*
		 * STEP 2: Compute the HMAC hash H with C as the message and K as the key (the
		 * HMAC algorithm is defined in the previous section, but also most
		 * cryptographical libraries support it). K should be passed as it is, C should
		...
		/*
		 * STEP 4: Take 4 bytes from H starting at O bytes MSB, discard the most
		 * significant bit and store the rest as an (unsigned) 32-bit integer, I.
		 */
		long i = 0;
		...
		/*
		 * STEP 5: The token is the lowest N digits of I in base 10. If the result has
		 * fewer digits than N, pad it with zeroes from the left.
		 */
		i %= 1000000;
	...
	private byte[] getKeyBytes(String key) {
		...
		String base32String = base32.encodeAsString(trimmed.toString().toUpperCase(Locale.ENGLISH).getBytes());
